...
- **Recency-base ranking.** Buzz channels have no working directory, so every Buzz session scores `repo_score=0`; `list` and `find` order purely by recency. For a complete `grep`, candidate channels are reranked by match density before the `--limit-sessions` cut (see below). Buzz results are inherently `--all-repos`.
