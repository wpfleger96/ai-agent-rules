...
**Review severity.** Findings are tagged **CRITICAL** (fundamental flaw, security risk, data-loss potential, incorrect approach), **IMPORTANT** (significant gap, missing consideration, maintainability concern), or **MINOR** (nice-to-have improvement, style issue, alternative worth considering). CRITICAL and IMPORTANT findings are blocking; MINOR are not. Every CRITICAL or IMPORTANT finding names the axis and the concrete defect.

**Reviewer independence.** Parallel reviewers form their assessments before reading each other's findings; the loop owner synthesizes them.

**Circuit breaker.** The agent who opened a review loop owns the pass count and announces the budget when opening the loop — **1 pass for Chore/Small tasks, 3 passes for Standard/Large**. If an axis is still sub-9 after the budget is exhausted, OR two reviewers disagree on whether a defect is real, the loop owner stops iterating and escalates to the user with: the defect, both positions, and their recommendation. Escalation is the only exit besides a cleared gate.

**Chore exemption.** Chores skip Thufir review — the implementer's correctness self-score is the gate; ship on green. Paul-dictated fixes where Paul supplied the exact text also skip review. **Exception:** a fix to a Thufir-flagged CRITICAL/IMPORTANT finding always gets re-review regardless of size — it inherits the risk of the defect it resolves.
</quality_gate>
